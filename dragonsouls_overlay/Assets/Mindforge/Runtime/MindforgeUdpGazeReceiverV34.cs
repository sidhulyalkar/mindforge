...
{
    /// <summary>
    /// Bounded, loopback-only inlet for derived gaze evidence. I/O stays off the Unity
    /// thread and each frame publishes at most the newest valid sample. Source-process
    /// timestamps are provenance only; freshness is measured from local socket receive time.
    /// </summary>
    [DefaultExecutionOrder(1210)]
    [DisallowMultipleComponent]
    public sealed class MindforgeUdpGazeReceiverV34 : MonoBehaviour
    {
        ...
        public event Action<MindforgeGazeEventV34> SampleReceived;
        ...
        private long _droppedOutOfOrder;
        ...
        public long DroppedOutOfOrder => Interlocked.Read(ref _droppedOutOfOrder);
        ...
        private void OnEnable()
        {
            ...
            {
                Debug.LogWarning($"[Mindforge:V34:Gaze] Could not bind UDP {port}: {ex.GetType().Name}");
                _client = null;
                return;
            }
        ...
        private void Update()
        {
            MindforgeGazeEventV34 newest = null;
            ...
            while (drained < maxDrainPerFrame && _messages.TryDequeue(out ReceivedPacket packet))
            {
                ...
                MindforgeGazeEventV34 sample;
                try
                {
                    sample = JsonUtility.FromJson<MindforgeGazeEventV34>(packet.Json);
                }
                catch
                {
                ...
                if (sample == null || !sample.HasSupportedSchema || !sample.IsFinite || !sample.HasSupportedOrigin)
                {
                    Interlocked.Increment(ref _droppedMalformed);
                    continue;
                }
                if (sample.seq <= _lastSeenSequence || sample.seq <= newestSequence)
                {
                    Interlocked.Increment(ref _droppedOutOfOrder);
                    continue;
                }
