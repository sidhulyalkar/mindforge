    ...
    public sealed class PhysicalArsenalMarkerBridge : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (combat != null)
            {
                ...
                combat.SwordProjectileParried += OnSwordParry;
        ...
        {
            if (sender == null) sender = UnityEngine.Object.FindObjectOfType<UdpGameMarkerSender>(true);
            ...
            if (guardIntegrity == null) guardIntegrity = GetComponent<GuardianStamina>();
        ...
        private void OnSwordParry(float reflectedDamage)
            => sender?.Emit(
                "SWORD_PARRY",
                "combat_outcome",
                reason: "PROJECTILE_REFLECTED",
                value: Mathf.Max(0f, reflectedDamage),
                bossPhase: Phase);

        private void OnGuardChanged(bool raised)
            => sender?.Emit(raised ? "SHIELD_RAISED" : "SHIELD_LOWERED", "combat_action", value: guardIntegrity != null ? guardIntegrity.Value : 0f, bossPhase: Phase);
        ...
        private void OnGuardBroken()
            => sender?.Emit("GUARD_BROKEN", "combat_outcome", value: guardIntegrity != null ? guardIntegrity.Value : 0f, bossPhase: Phase);
