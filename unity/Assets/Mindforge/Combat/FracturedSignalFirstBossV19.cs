...
namespace Mindforge.Combat
{
    ...
    ///
    /// V0.28 also owns the encounter's minimum actor-separation contract. Separation is handled
    /// here, inside the existing movement authority, rather than by a competing push component.
    /// It can break an ordinary attack/orbit hold only when the Guardian has penetrated the
    /// creature's readability envelope. Neural calibration/resonance still freezes all movement.
    /// </summary>
    ...
    public sealed class FracturedSignalFirstBossV19 : MonoBehaviour
    {
        ...
        [Header("V0.28 readability separation")]
        [SerializeField] private float minimumSeparationDistance = 2.75f;
        [SerializeField] private float emergencySeparationSpeed = 4.8f;
        [SerializeField] private float separationHysteresis = 0.22f;
        ...
        public float MinimumSeparationDistance => Mathf.Max(1.4f, minimumSeparationDistance);
        public bool EmergencySeparating => _separating;
        ...
        private void FixedUpdate()
        {
            ...
            // Readability has priority over ordinary attack/orbit holds. If the Guardian enters
            // the creature's body envelope, the existing locomotion authority backs the boss out
            // until silhouettes are distinct again. This does not cancel or reschedule attacks.
            ...
            float preferred = Mathf.Max(PreferredDistance(), MinimumSeparationDistance + 0.9f);
            ...
            {
                desiredDirection = Vector3.Slerp(-toward, lateral, 0.18f).normalized;
            ...
            candidate.y = _home.y;
            candidate = ClampToHomeLeash(candidate);

            if (!PositionClear(candidate))
            {
                Vector3 alternate = transform.position - lateral * speed * Time.fixedDeltaTime;
                alternate.y = _home.y;
                alternate = ClampToHomeLeash(alternate);
                if (PositionClear(alternate)) candidate = alternate;
                else candidate = transform.position;
            }

            ApplyMovement(candidate, speed);
            FacePlayer();
        }

        private bool TryEmergencySeparation()
        {
            if (_player == null) return false;
            Vector3 fromPlayer = Vector3.ProjectOnPlane(transform.position - _player.position, Vector3.up);
            float distance = fromPlayer.magnitude;
            float enter = MinimumSeparationDistance;
            float exit = enter + Mathf.Max(0.05f, separationHysteresis);

            if (!_separating && distance >= enter) return false;
            if (_separating && distance >= exit)
            {
                _separating = false;
                return false;
            }
            ...
            _separating = true;
            Vector3 away = distance > 0.02f ? fromPlayer / distance : -transform.forward;
            float penetration = Mathf.Max(0f, exit - distance);
            float speed = Mathf.Max(MoveSpeed(), emergencySeparationSpeed) * Mathf.Lerp(1f, 1.35f, Mathf.Clamp01(penetration / enter));
            Vector3 candidate = transform.position + away * speed * Time.fixedDeltaTime;
            candidate.y = _home.y;
            candidate = ClampToHomeLeash(candidate);

            if (!PositionClear(candidate, ignoreGuardian: true))
            {
                Vector3 tangent = Vector3.Cross(Vector3.up, away) * _orbitSide;
                Vector3 sidestep = transform.position + (away * 0.72f + tangent * 0.55f).normalized * speed * Time.fixedDeltaTime;
                sidestep.y = _home.y;
                sidestep = ClampToHomeLeash(sidestep);
                candidate = PositionClear(sidestep, ignoreGuardian: true) ? sidestep : transform.position;
            }
        ...
        private Vector3 ClampToHomeLeash(Vector3 candidate)
        {
            Vector3 fromHome = Vector3.ProjectOnPlane(candidate - _home, Vector3.up);
            float leash = Mathf.Max(2f, homeLeashRadius);
            if (fromHome.magnitude <= leash) return candidate;
            Vector3 planar = _home + fromHome.normalized * leash;
            planar.y = _home.y;
            return planar;
        }

        private void ApplyMovement(Vector3 candidate, float speed)
        {
        ...
        private bool PositionClear(Vector3 candidate, bool ignoreGuardian = false)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (ignoreGuardian && _player != null && (t == _player || t.IsChildOf(_player) || _player.IsChildOf(t))) continue;
