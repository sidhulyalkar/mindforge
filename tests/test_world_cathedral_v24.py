...
def test_v24_is_the_final_canonical_world_layout_stage_before_v25_and_v26_presentation():
    latest = read(LATEST)
    source = read(BUILDER)
...
def test_v24_palette_is_light_cathedral_first_not_dark_blockout_first():
    ...
    for token in (
        'Assets/Mindforge/Generated/V24/Materials',
        'Assets/Mindforge/Generated/V24/Textures',
        '"IvoryStone"',
        '"WhiteMarble"',
        '"PaleFloor"',
        '"CoolShadowStone"',
...
def test_v24_uses_one_floor_visual_language_over_existing_collision_authority():
    ...
    for token in (
        ...
        '"ChoirRampAisle"',
        '"Threshold_',
        "ramp.localScale.y * 0.5f + 0.020f",
        "WorldFoundationV23Builder.AscentSlopeDegrees",
    ):
        assert token in source
...
def test_v24_module_kit_is_semantic_and_reusable():
    ...
    for token in (
        ...
        "LumenSconce(",
        "BeamBetween(",
        "ProductionMeshLibraryV09.FlutedColumn()",
        "ProductionMeshLibraryV09.PointedArch()",
        "AddComponent<CathedralRoleV24>()",
    ):
...
def test_v24_composes_named_cathedral_zones_with_repeated_architectural_rhythm():
    ...
    for token in (
        ...
        '"V24_Market_Cloister"',
        '"V24_Choir_Ascent"',
        '"V24_Fractured_Signal_Apse"',
        '"V24_Vault_Rhythm"',
        '"CathedralColumn_Sanctum_',
        '"CathedralColumn_Nave_',
        '"CathedralColumn_Cloister_',
        '"CathedralColumn_Apse_',
        '"CathedralArch_Sanctum_',
        '"CathedralArch_Nave_',
        '"CathedralArch_Cloister_',
        '"CathedralArch_Choir_',
        '"CathedralVaultRib_',
    ):
        assert token in source
...
def test_v24_builder_has_fail_closed_visual_structure_validation():
    ...
    for token in (
        "ValidateCathedral",
        "renderer.sharedMaterial != palette.PaleFloor",
        "floors < 12 || supports < 45 || mystic < 8",
        "has no cathedral structural role",
        "noisy legacy layer still active",
        'FindDeep(canonicalRoot, "AscentUnderlay") != null',
    ):
        assert token in source
...
def test_v24_native_smoke_docs_and_guids_are_present():
    ...
    doc = read(DOC)
    for token in (
        "V24CathedralRole_IsPureSemanticMarker",
        "V24CathedralModuleKit_CanConstructSemanticGeometry",
    ...
    paths = (
        EDITOR / "CathedralMaterialLibraryV24.cs.meta",
        EDITOR / "CathedralModuleLibraryV24.cs.meta",
        EDITOR / "WorldCathedralV24Builder.cs.meta",
        WORLD / "CathedralRoleV24.cs.meta",
