...
from mindforge_neuro.playtest import finalize_playtest_bundle, sha256_file


def marker(seq: int, event: str, *, session: str = "session-a", t: float = 0.0) -> GameMarker:
    return GameMarker(
        ...
        frame=seq,
        fixed_tick=seq * 2,
        reason="CONTROLLER_ONLY_NO_BCI" if event == "QUALIFICATION_MODE" else None,
    )
...
def test_finalize_playtest_bundle_hashes_and_summarizes_one_session(tmp_path: Path):
