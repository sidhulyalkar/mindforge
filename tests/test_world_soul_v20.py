...
def test_world_soul_builds_one_continuous_environment_grammar():
    world = read(WORLD)
    for token in (
        '"WestLandmass"', '"EastLandmass"', '"SouthLandmass"', '"NorthHighlands"',
        '"WorldSoul_Sanctum_Grove"', '"WorldSoul_Causeway_Banks"', '"WorldSoul_Market_Ruins"',
        '"WorldSoul_Ascent_Geology"', '"WorldSoul_Fracture_Crater"', '"WorldSoul_Distant_City"',
        '"WorldSoul_Horizon_Landmarks"', "RetextureCanonicalArchitecture", "BuildAncientTree",
        "ScatterNaturalRock", "ConfigureAtmosphereAndLighting",
    ):
...
def test_world_soul_is_editor_authored_static_presentation_not_gameplay_authority():
    ...
    for forbidden in (
        ...
        "UdpNeuralReceiver", "ReceiveDamage(", "SetExternalPause(", "TryApply(", "MarkResolved(",
        "AddComponent<Collider", "AddComponent<Rigidbody", "UnityEngine.Random",
    ):
...
def test_world_surfaces_reuse_production_triplanar_pbr_with_generated_normals():
    ...
    for token in (
        'Assets/Mindforge/Generated/V20/Materials', 'Assets/Mindforge/Generated/V20/Textures',
        "SurfaceRevision = 2", "TextureWrapMode.Repeat", "FilterMode.Trilinear",
        "ProductionMaterialAuthoringV09.TriplanarShaderPath", "ProductionMaterialAuthoringV09.TriplanarShaderName",
        'material.SetTexture("_BaseMap", surface.Albedo)', 'material.SetTexture("_BumpMap", surface.Normal)',
        'material.SetFloat("_MetersPerTile"', 'material.SetFloat("_BlendSharpness"',
        'material.SetFloat("_NormalFadeDistance"', "ShaderUtil.ShaderHasError", "material.enableInstancing = true",
        'Shader.Find("Universal Render Pipeline/Lit")', 'Shader.Find("Skybox/Procedural")',
    ):
...
def test_world_mesh_recipes_are_generated_cached_assets_not_binary_source_art():
    meshes = read(MESHES)
    for token in (
        'Assets/Mindforge/Generated/V20/Meshes', "MeshRevision = 1", "TerrainCache", "RockCache",
        "TerrainPatch(", "RockVariant(", "RecalculateNormals()", "RecalculateTangents()", "EditorUtility.CopySerialized",
    ):
...
def test_world_soul_tracks_public_graphics_references_without_vendoring_runtime_packages():
    ...
    for forbidden in ("jp.keijiro.noiseshader", "Packages/", "com.github", "AssetBundle", ".fbx", ".blend"):
...
def test_world_soul_editor_scripts_have_pinned_unique_unity_guids():
    scripts = ("WorldSoulNoiseV20.cs", "WorldSoulMaterialLibraryV20.cs", "WorldSoulMeshLibraryV20.cs", "WorldSoulV20Builder.cs")
    guids = []
