...
def test_v23_remains_the_foundation_stage_before_later_visual_layers():
    latest = read(LATEST)
...
def test_v23_removes_the_recording_visible_crossing_ascent_slab():
    ...
    for token in (
        "AscentSlopeDegrees = -8.1f", 'underlayRoot.Find("AscentUnderlay")', "DestroyImmediate(stale.gameObject)",
        '"V23_Ascent_Visual_Reconciliation"', '"AscentFoundationSkin"', "new Vector3(AscentSlopeDegrees, 0f, 0f)",
    ):
...
def test_v23_fills_the_known_causeway_market_hole_with_one_visible_collision_surface():
    source = read(BUILDER)
    for token in (
        '"V23_Route_Transition_Caps"', '"CausewayMarketTransition"', "causeway ends at z=32", "market starts at z=33",
        "new Vector3(0f, -0.05f, 32.5f)", "new Vector3(8.6f, 0.10f, 1.10f)", "palette.WornStone",
        'Require(root, "V23_Route_Transition_Caps/CausewayMarketTransition")', "transition.GetComponent<BoxCollider>() == null",
    ):
...
def test_v23_keeps_recessed_guards_below_authoritative_route_surfaces():
    source = read(BUILDER)
    for token in (
        '"V23_Collision_Reconciliation"', '"LowerRouteSeamGuard"', '"AscentSeamGuard"', '"BossArenaSeamGuard"',
        "BoxCollider collider = go.AddComponent<BoxCollider>()", "guards.GetComponentsInChildren<BoxCollider>(true).Length != 3",
    ):
        assert token in source
...
def test_v23_makes_all_four_generated_outer_landmasses_physically_explorable():
    source = read(BUILDER)
    for token in (
        "ReconcileOuterTerrainCollision", 'worldRoot + "WestLandmass"', 'worldRoot + "EastLandmass"',
        'worldRoot + "SouthLandmass"', 'worldRoot + "NorthHighlands"', "terrain.gameObject.AddComponent<MeshCollider>()",
        "collider.sharedMesh = filter.sharedMesh", "collider.convex = false", "terrainCollider.sharedMesh != terrainFilter.sharedMesh",
    ):
        assert token in source
...
def test_v23_reconciles_large_visual_solids_without_turning_patina_into_obstacles():
    source = read(BUILDER)
    for token in (
        "AddStructuralContactProxies", "ShouldReceiveContactProxy", '"SanctumColumn_"', '"CausewayPylon"',
        '"MarketColumn_"', '"AscentColumn"', '"FractureSpire_"', '"FieldRock_"', '"CraterRock_"',
        '"WallShoulder_"', '"ChamberButtress_"', "size.x * 0.72f", "size.y * 0.90f", "size.z * 0.72f",
    ):
        assert token in source
    proxy_section = source[source.index("private static bool ShouldReceiveContactProxy") : source.index("private static void RebuildInwardCavernCeiling")]
    for forbidden_solid in ('"Reed_"', '"Fern_"', '"Shrub_"', '"ArenaFracture_"', '"RouteLumen_"'):
...
def test_v23_cavern_mesh_faces_inward_and_render_collision_share_topology():
    ...
    meshes = read(MESHES)
    for token in (
        "WorldFoundationMeshLibraryV23.InwardTerrainPatch", "filter.sharedMesh = inward", "collider.sharedMesh = null",
        "collider.sharedMesh = inward", "mesh.normals[centre].y > -0.20f",
    ):
        assert token in builder
    for token in (
        "InwardTerrainPatch(", "BuildTransientInwardPatch(", "triangles.Add(a); triangles.Add(c); triangles.Add(d);",
        "triangles.Add(a); triangles.Add(b); triangles.Add(c);", "RecalculateNormals()", "RecalculateTangents()",
        "MeshRevision = 1", "Assets/Mindforge/Generated/V23/Meshes",
    ):
...
def test_v23_seals_high_vault_ends_and_visually_roots_the_route():
    source = read(BUILDER)
    for token in (
        '"V23_Cavern_Upper_End_Seals"', '"SouthUpperBacking"', '"NorthUpperBacking"', '"UpperSealRock_',
        '"V23_Route_Foundations"', '"CausewayRetainerL"', '"CausewayRetainerR"', '"MarketRetainerL"',
        '"MarketRetainerR"', '"AscentFoundationRock_',
    ):
        assert token in source
...
def test_v23_is_static_editor_authoring_not_new_runtime_gameplay_authority():
    ...
    for forbidden in (
        ...
        "Time.deltaTime", "Time.unscaledDeltaTime", "NeuralEvent", "UdpNeuralReceiver", "ReceiveDamage(",
        "SetExternalPause(", "TryApply(", "MarkResolved(", "AddComponent<Rigidbody", "UnityEngine.Random",
    ):
...
def test_v23_has_native_inward_mesh_smoke_documentation_and_pinned_unique_guids():
    ...
    paths = (
        EDITOR / "WorldFoundationV23Builder.cs.meta", EDITOR / "WorldFoundationMeshLibraryV23.cs.meta",
