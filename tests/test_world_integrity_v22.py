...
def test_v22_remains_the_integrity_stage_before_later_world_and_presentation_layers():
    latest = read(LATEST)
...
def test_v22_forces_structural_surfaces_back_to_opaque_depth_writing_state():
    source = read(WORLD)
    for token in (
        'material.SetFloat("_Surface", 0f)', 'material.SetFloat("_Blend", 0f)',
        'material.SetFloat("_SrcBlend", (float)BlendMode.One)', 'material.SetFloat("_DstBlend", (float)BlendMode.Zero)',
        'material.SetFloat("_ZWrite", 1f)', 'material.SetFloat("_AlphaClip", 0f)',
        'material.DisableKeyword("_SURFACE_TYPE_TRANSPARENT")', 'material.DisableKeyword("_ALPHAPREMULTIPLY_ON")',
        'material.DisableKeyword("_ALPHATEST_ON")', 'material.SetOverrideTag("RenderType", "Opaque")',
        'material.renderQueue = (int)RenderQueue.Geometry', 'material.SetShaderPassEnabled("ShadowCaster", true)',
...
def test_v22_closes_ground_roof_walls_and_far_world_boundary():
    source = read(WORLD)
    for token in (
        '"V22_Continuous_Ground_Underlay"', '"LowerRouteUnderlay"', '"AscentUnderlay"', '"BossPlateauUnderlay"',
        '"V22_Cavern_Vault"', '"CavernVaultUnderside"', '"WestCavernBackwall"', '"EastCavernBackwall"',
        '"SouthCavernBackwall"', '"NorthCavernBackwall"', '"V22_Traversal_Envelope"', '"WestBoundary"',
        '"EastBoundary"', '"SouthBoundary"', '"NorthBoundary"', "WorldSoulMeshLibraryV20.TerrainPatch",
        "MeshCollider roofCollider = roof.AddComponent<MeshCollider>()", "BoxCollider collider = go.AddComponent<BoxCollider>()",
    ...
    for forbidden in (
        ...
        "Time.deltaTime", "Time.unscaledDeltaTime", "NeuralEvent", "UdpNeuralReceiver", "ReceiveDamage(", "UnityEngine.Random",
    ):
...
def test_v22_ties_cavern_architecture_into_route_and_boss_chamber():
    source = read(WORLD)
    for token in (
        '"V22_Vault_Wall_Transitions"', '"VaultRib_', "ProductionMeshLibraryV09.PointedArch()",
        '"V22_Fractured_Signal_Chamber"', '"ChamberButtress_', '"ChamberCrownRib_',
        '"V22_Route_Luminance_Anchors"', '"RouteLumen_',
    ):
        assert token in source
...
def test_v22_boss_uses_most_of_the_chamber_and_reduces_projectile_noise():
    ...
    for field in (
        "phaseOnePreferredDistance", "phaseTwoPreferredDistance", "phaseThreePreferredDistance", "distanceBand",
        "phaseOneMoveSpeed", "phaseTwoMoveSpeed", "phaseThreeMoveSpeed", "retreatMultiplier", "orbitBias",
        "turnSharpness", "homeLeashRadius", "collisionProbeRadius", "postAttackRecovery", "orbitSideHoldSeconds",
    ):
        assert f'CanSet<FracturedSignalFirstBossV19, float>("{field}")' in source
        assert f"private float {field}" in v19
    for token in (
        'Set(_movement, "homeLeashRadius", 14.2f)', 'Set(_movement, "collisionProbeRadius", 0.58f)',
        'Set(_movement, "orbitSideHoldSeconds", 1.55f)', 'Set(_director, "phaseOneInterval", 2.35f)',
        'Set(_director, "phaseTwoInterval", 2.02f)', 'Set(_director, "phaseThreeInterval", 1.72f)',
        'Set(_director, "radialCount", 6)', 'Set(_director, "maxEchoes", 1)',
    ):
        assert token in source
...
def test_v22_melee_is_readable_and_sword_contact_has_trigger_only_hull():
    ...
    melee = read(MELEE)
    for field in (
        "engageDistance", "cleaveRange", "cleaveArcDegrees", "cleaveTelegraphPhaseOne", "cleaveTelegraphPhaseTwo",
        "cleaveTelegraphPhaseThree", "slamRadius", "slamTelegraphPhaseTwo", "slamTelegraphPhaseThree",
    ):
        assert f'CanSet<FracturedSignalMeleeDirector, float>("{field}")' in source
        assert f"private float {field}" in melee
    for token in (
        '"V22_BossCombatHull"', "CapsuleCollider collider = hull.AddComponent<CapsuleCollider>()",
        "collider.isTrigger = true", "collider.radius = 1.08f", "collider.height = 3.15f",
    ):
        assert token in source
...
def test_v22_stall_recovery_waits_for_commitment_and_preserves_real_pause_owners():
    ...
    link = read(LINK)
    for token in (
        "AttackTelegraphed += OnAttackTelegraphed", "AttackFired += OnAttackFired", "Time.unscaledTime < _commitUntil",
        "stallWindowSeconds = 0.85f", "KickOrbitRecovery()", 'GetField(\n                "_orbitSide"',
        'GetField(\n                "_nextOrbitSwap"', 'GetField(\n                "_holdUntil"', "_wispIntermission.Active",
        "_linkContingency.Degraded", "_linkContingency.ParticipantStopped", "_director.SetExternalPause(false)",
...
def test_v22_has_native_unity_construction_smoke_and_pinned_guids():
    ...
    paths = (EDITOR / "WorldIntegrityV22Builder.cs.meta", COMBAT / "FracturedSignalDuelStabilityV22.cs.meta")
    guids = []
    for path in paths:
        text = read(path)
